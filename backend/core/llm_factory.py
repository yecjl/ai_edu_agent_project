...
class LLMFactory:
    ...
    def _build_model_kwargs(cls, model_key: str) -> dict[str, Any]:
        ...
        return {
            ...
            "api_key": settings.llm_api_key,       # 来自 .env.local
            "base_url": settings.llm_base_url,     # DeepSeek 接口地址
            ...
            "extra_body": {
                "thinking": {"type": "disabled"}   # 禁用模型层思考动画, 启动:enabled, 禁用:disabled
            }
        }
    ...
    def get_structured_llm(
        cls,
        agent_type: str,
        output_schema: Type[BaseModel],   # 期望的输出结构（一个 Pydantic 模型类, 是类, 不是对象）
        temperature: float = 0,
    ) -> Runnable:
...
if __name__ == '__main__':
    ...
    async def main():
        ...
        llm = get_structured_llm("qa", PersonInfo)
        resp:PersonInfo = await llm.ainvoke([HumanMessage(content="你负责从文本中抽取人物信息."), HumanMessage(content="我叫小明, 今年 25岁, 住在上海.")])
        print("DeepSeek 回复:", resp)  # BaseModel对象输出
        print("DeepSeek 回复:", resp.model_dump())  # dict 输出
